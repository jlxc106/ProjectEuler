#! /usr/bin/python
# problem statement: projecteuler.net/problem=11
...
for row in range(20):
	for col in range(20):
		if myArray[row][col]==0:
			continue
		if row + 3 < 20:
			temp = myArray[row][col]*myArray[row+1][col]*\
			myArray[row+2][col]*myArray[row+3][col]
		if temp > sum:
			sum = temp
		if col + 3 < 20:
			temp = myArray[row][col]*myArray[row][col+1]*\
			myArray[row][col+2]*myArray[row][col+3]
		if temp > sum:
			sum = temp
		if col + 3 < 20 and row + 3 < 20:
			temp = myArray[row][col]*myArray[row+1][col+1]*\
			myArray[row+2][col+2]*myArray[row+3][col+3]
		if temp > sum:
			sum = temp
